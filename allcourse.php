...
<body>
	...
	<nav class="course">
        <?php
		 echo "<br><ul>";
		 $stmt1 = $dbh->prepare("select * from course;");
	     if($stmt1->execute(array())){
		    while ($row = $stmt1->fetch()) {
		    	echo "<li>";
				echo "<a href=course.php?cid=".$row['cid']."><img  src=images/".$row['cid']. " width=150 height=150 /></a>";
                echo "<div id=courseinfo><a href=course.php?cid=".$row['cid']."><h3>".$row['name']."</h3></a></div><br>";
			}
		 }
		?>
	</nav>
